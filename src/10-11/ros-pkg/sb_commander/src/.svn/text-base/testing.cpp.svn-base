...
void ir_state_callback(const sensor_msgs::LaserScanConstPtr& msg_ptr)
{
   data.GetConstants(msg_ptr->range_max, msg_ptr->range_min, msg_ptr->angle_max, msg_ptr->angle_min, msg_ptr->angle_increment);		
	...
	path.FindClosestCone(data.left, data.right, data.middle_left, data.middle_right); // left, right, m_left, m_right
	path.DeterminePath();
	...
	if (my_count == 10)
	{
		//data.PrintDistanceAngle();
		//data.PrintCone();
		path.PrintClosestCone();
		my_count = 0;
	}
	
	//path.PrintClosestCone();
	//path.PrintDirection();
	//path.PrintConeDirection();
	data.Clear();
	path.Clear();
	my_count++;
}
...
sb_msgs::CarCommand driver()
{
   ...
   // cout << "Throttle: " << car_msg.throttle << endl;
    //cout << "Steering: " << car_msg.steering << endl;
