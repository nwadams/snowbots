...
//	double irLeftAngle  = laserScanMsg->angle_max;	//angle of IR sensor to left  (rad)
	...
	double dangerZone = 0.85;
	double irFront = 0;
	double irRight = 0;
	double irLeft = 0;

	for (int i=11*NUM_RAYS/24; i < 13*NUM_RAYS/24; i++)
	{
		if (laserScanMsg->ranges[i] < dangerZone)
		{
			irFront = laserScanMsg->ranges[i];
			break;
		}
		irFront = laserScanMsg->ranges[i];
	}

	for (int i=0; i < NUM_RAYS/4; i++)
	{
		if (laserScanMsg->ranges[i] < dangerZone)
		{
			irRight = laserScanMsg->ranges[i];
			break;
		}
		irRight = laserScanMsg->ranges[i];
	}

	for (int i = 3*NUM_RAYS/4; i < NUM_RAYS; i++)
	{
		if (laserScanMsg->ranges[i] < dangerZone)
		{
			irLeft = laserScanMsg->ranges[i];
			break;
		}
		irLeft = laserScanMsg->ranges[i];	
	}
