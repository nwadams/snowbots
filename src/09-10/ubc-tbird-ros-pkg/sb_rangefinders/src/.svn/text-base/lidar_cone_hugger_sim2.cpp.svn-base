...
/* Global Constants */
int NUM_RAYS = 655;	//Number of lidar-rays emmitted
double FIELD = 3.5;  	//Maximum radius for creating the imaginary wall (meters)
double MAX_DISTANCE = 4.00;	//FIELD < MAX_D
int MAX_STEERING = 30.0;	//Degrees

/* Function Declaration */
void irCallBack(const sensor_msgs::LaserScanConstPtr&);
double calculateSteering(double theta, int sharpness);
...
{	
  ...
  if(argc != 3){
    std::cout << "Usage: Nodename <FieldView> <MaxDistance>" << std::endl;
    return -1;
  }
...
// std::cout << "# rays: " << (angle_max-angle_min)/(angle_increment) << std::endl;
   ...
   for (int i=0; i < NUM_RAYS; i++)
   {
      ...
      if(myRanges[i] < 0.01 || myRanges[i] > 4.0){
...
*/
      ...
      {           
         ...
         {
            if(myRanges[point1] < FIELD)
              myRanges[j] = (myRanges[point1]+myRanges[point2])/2;
            else
              myRanges[j] = myRanges[point2];
         }
         point1 = point2+1;
      }
   }
   ...
   for(int i = 0; i < NUM_RAYS; i++)
   {
      if (i < NUM_RAYS/2)
      {
         ...
         fxcomponents[i] = myRanges[i] * cos((angle_min+angle_increment) + (i * angle_increment));
         xcomponents[i] = myRanges[i] * sin((angle_min+angle_increment) + (i * angle_increment));
      }
      else
      {
  ...
  if(area == 0)
   ...
   if(myRanges[NUM_RAYS/2] < 2.0)
     sharpness = 3;
   else if(myRanges[NUM_RAYS/2] < 3.0)
     sharpness = 2;
   else
     sharpness = 1;

   double steering = calculateSteering(theta, sharpness);
   ...
   /* Backing up */
   for (int i=NUM_RAYS*5/11; i < NUM_RAYS*6/11; i++)
   {
      if (myRanges[i] < 0.6)
      {
   ...
   double irRight = myRanges[0];
   double irLeft  = myRanges[NUM_RAYS-1];
