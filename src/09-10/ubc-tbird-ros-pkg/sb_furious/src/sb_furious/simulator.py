"""
This module contains the L{FuriousSimulator} class that can be used to
simulate a Furious microcontroller.  It does this by opening a connection
on a pseudo-tty (eg, /dev/pts/3) and listening for commands on that.
...
from devices import FuriousBoard

class FuriousSimulator(object):
  """
  ...
  def encode(self):
    ...
    for val in self._state:
      out += str(val)
      out += ","
    out += FuriousBoard.COMMAND_DELIM
    return out 
