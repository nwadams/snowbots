...
void onStateUpdate(const sb_msgs::RobotStateConstPtr& new_state)
{
  ...
  double acceleration = accel.getAccel( new_state->analog[accel_port] );
  double speed = spd_ctrl.getSpeed(acceleration, time);
  double throttle = spd_ctrl.getThrottle(speed);
  throttle_cmd.pwm = throttle_ctl.getRawValue(throttle);
  servo_pub.publish(throttle_cmd);
  ROS_INFO("%6.3f, %6.3f, %6.3f, %6.3f, %i", 
      time, acceleration, speed, throttle, throttle_cmd.pwm);
