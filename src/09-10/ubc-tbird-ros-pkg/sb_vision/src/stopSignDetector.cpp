/**
 * stop_sgin_detector Node
 * 
 * Description:
 *   This node takes the frames from a camera and analyises them to detect a
 *   stop sign. The output of this node is a message of type "Float64" which
 *   contains a confidence percentage (0 to 1) of stop sign existance.
 *
...
const std::string NODE_NAME     = "stop_sign_detector";
const std::string PUBLISH_TOPIC = "stop_sign_state";
const std::string IMG_PUB_TOPIC = "cv_bridge";
...
int main(int argc, char *argv[])
{
  ...
  ros::NodeHandle n;
  ros::Publisher dataPub = n.advertise<std_msgs::Float64>(PUBLISH_TOPIC, MSG_QUEUE_SIZE);
  ...
  uchar *data;
    ...
    if(argc != 3){ 
      printf("ERROR: Wrong number of input parameters\nUsage: stopSignDetector <red_threshold50> <min_ratio_size200>\n");
      return -1; 
    }
    redThreshold = atoi(argv[1]);
    MIN_RATIO_SIZE = atoi(argv[2]);

    /* check if a picture is being passed */
    CvCapture* capture = cvCaptureFromCAM( CV_CAP_V4L +1);
    if( !capture ) {
    ...
    while( ros::ok() )
    {
      ...
      /* Publishing out the image as a ROS message */
      sensor_msgs::CvBridge image;
      image_pub.publish(image.cvToImgMsg(originalImg,"bgr8"));
      //ROS_INFO("I published a frame to cv_brdige");     
      /*********************************************/

      // get the image data
      height    = 3 * originalImg->height / 5;
      ...
      msg.data = confidence;
      dataPub.publish(msg);
      ROS_INFO("I published a message: Confidence = %f", msg.data);
...
/****************************** IS STOP SIGN ?! *******************************/
  ...
  if((height*width)/redPixVector.size() > MIN_RATIO_SIZE){
  ...
  std::cout << "Parallel: " << parallel << std::endl;
    ...
    std::cout << "No red pixel found... No way there is a stop sign!" << std::endl;
    certainty = 0;
  }
...
std::vector<Line> detectEdges(uchar *temp, std::vector<Point>& boundaryPoints)
{
  ...
  const double MAX_INTERCEPTION_RANGE = 40;
