 ...
 */
geometry_msgs::Twist driver(ros::Time begin)
{
    sb_msgs::CarCommand car_msg;

    // Begin writing here
	double Steering = (0.5/3.14159)*atan((cos(22.5)*(state.NE - state.NW) + cos(45)*(state.E - state.W))/5);//scale is from -1 to 1
	double Throtle = (0.5/3.14159)*atan((state.N+cos(22.5)*(state.NW + state.NE) + cos(45)*(state.W+state.E))/5);//scale is from 0 to 1
    // End writing
...
int main( int argc, char** argv )
{
    ...
    while(ros::ok())
    {
        ...
        geometry_msgs::Twist twist_msg = driver(begin);
