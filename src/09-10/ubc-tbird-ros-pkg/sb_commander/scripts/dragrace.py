...
def theDriver():
  ...
  while not rospy.is_shutdown():
    ...
    if (time.time() - starttime < 0.5) :
      ...
      throt = 0.50
    elif (time.time() - starttime > 3.0) :
      throt = -0.25
    elif (time.time() - starttime > 4.0) :
      throt = 0
    else :
      throt = 0.70
    
    
    #if abs(lidar_direction) < 25:
     # steer = lidar_direction/1500.0 
    #else :
     # steer = lidar_direction/600.0
